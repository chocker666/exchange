...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i< M; i++){
      for(int j= 0; j< N; j++){
        if(abs(i-30) < 20 && abs(j-60) < 10)
          DATA[i][j] = std::to_string(((int)sqrt((i-30)*(i-30)+(j-60)*(j-60)))*250/60);
        else
          DATA[i][j] = std::to_string(12);
      }
    }
    ...
    while(ros::ok()){
        ...
        ROS_INFO("got S%c%c",CMD[1], CMD[2]);
        switch (CMD[1]){
          case 'T':
            switch(CMD[2]){
              case '0':
                ...
                break;
              case '1':
                send_repo += "1:1,2:6642,3:230";
                break;
              case '2':
                send_repo += "1:70,2:10,3:5";
                break;
              case '3':
                send_repo += "1:70,2:0.5";
                break;
              case '4':
                send_repo += "DONE";
                break;
          ...
          case 'C':
            switch(CMD[2]){
              case '0':
                send_repo += "102,333,259-610,282,555-80";
                break;
              case '1':
                for(int i=0; i<M; i++){
                  for(int j=0; j<N; j++){
